...
class EditableTableWidget(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        self._data_table = DataTable(id="data_table_widget", fixed_columns=0)
    ...
    def on_mount(self) -> None:
        """控件首次挂载时设置 DataTable。"""
        assert self._data_table is not None, "DataTable 未在 compose 中初始化"
        ...
        for label_text in header_labels:
            self._data_table.add_column(label_text, width=column_width)

        data_rows = self.table_data[1:]
        for row_values in data_rows:
            # DataTable 会自动为没有提供 key 的行生成唯一的键
            self._data_table.add_row(*[str(cell_value) for cell_value in row_values])
    ...
    async def on_key(self, event: events.Key) -> None:
        ...
        if event.key == "enter":
            if self._data_table.has_focus:
                cursor_coord = self._data_table.cursor_coordinate
                # 确保光标在有效的数据行上
                if cursor_coord.row < 0 or cursor_coord.row >= self._data_table.row_count:
                    return
        ...
        if self._cell_to_edit_coords:
            ...
            if 0 <= source_data_row_idx < len(self.table_data) and \
               0 <= table_col_idx < len(self.table_data[source_data_row_idx]):
                ...
                self._data_table.update_cell_at(self._cell_to_edit_coords, new_value)
    ...
    def action_add_new_row(self) -> None:
        ...
        new_row_index = self._data_table.row_count - 1
        if new_row_index >= 0:
    ...
    def action_delete_selected_row(self) -> None:
        ...
        if self._data_table.row_count > 0 and 0 <= cursor_row_idx < self._data_table.row_count:
            all_row_keys = list(self._data_table.rows.keys())
            key_to_delete = all_row_keys[cursor_row_idx]
            self._data_table.remove_row(key_to_delete)
        
            del self.table_data[cursor_row_idx + 1]
        
            if self._data_table.row_count > 0:
                new_cursor_row = min(cursor_row_idx, self._data_table.row_count - 1)
                current_col_cursor = self._data_table.cursor_column
                # 使用 len(self._data_table.columns) 替换 self._data_table.column_count
